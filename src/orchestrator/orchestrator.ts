...
export class Orchestrator {
  ...
  async dispatch(workstreamId: string, instruction: string) {
    ...
    const executionInstruction = this.buildExecutionInstruction(dispatchWorkstream, instruction);
  ...
  async generatePlan(workstreamId: string, instruction: string, trigger: "manual" | "auto" = "manual") {
    ...
    const plan = await adapter.startTurn({
      ...
      instruction: buildPlanningInstruction({
        ...
        epicCharter: project.metadata?.epicCharter ?? null,
      }),
      addDirs: [cwd],
      maxTurns: 3,
      permissionMode: "plan",
  ...
  private buildExecutionInstruction(workstream: WorkstreamRow, instruction: string): string {
    if (!workstream.plan || workstream.current_goal !== instruction) {
      return instruction;
    }
